...
$sql="SELECT care_person.selian_pid, care_person.pid, name_first, name_last, sex, batch_nr, date_birth, care_encounter.encounter_nr,send_date,dept_nr,
	care_test_request_".$db_request_table.".create_id FROM care_person LEFT JOIN care_encounter ON care_person.pid=care_encounter.pid
...
$person=$result->FetchRow();
				$h_pid=$person['pid'];
				$h_batch_nr=$person['batch_nr'];
				$h_encounter_nr=$person['encounter_nr'];
				$h_selian_file_number=$person['selian_pid'];
				$h_name_first=$person['name_first'];
				$h_name_last=$person['name_last'];
				$h_birthdate=$person['date_birth'];
		        $h_sex=$person['sex'];
		        $h_DoctorID=$person['create_id'];
