<?php
session_start();
$bill_obj->Display_Header($LDNewQuotation,$enc_obj->ShowPID($batch_nr),''); 
?>
<?php 
...
function chkform(form) {
	...
	var clearcheck;
	    ...
	    user='<?php echo $_SESSION['sess_user_name']?>';
		...
		if(balance.value!="" && clearcheck.checked==false){
			alert("PLEASE PUT TICK THERE IS A BALANCE");
			return false;
			}
		else	
...
<input type="hidden" value="<?php echo $bill_obj->is_discharged($_REQUEST['encounter_nr']);?>" id="discharged" name="discharged">
					  </td>
...
if($bill_obj->GetAdvanceTotalAmount($_REQUEST['encounter_nr'])>0){
echo 'document.getElementById(\'balance\').value=totalsum-'.$bill_obj->GetAdvanceTotalAmount($_REQUEST['encounter_nr']).'';
}
					  			?>
					  	}
