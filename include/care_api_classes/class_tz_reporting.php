...
//------------------------------------------------------------------------------------------------------
	...
	function DisplayPharmacyTableHead($class,$admission,$bill){
		global $PRINTOUT;
		global $LDPharmacyReportwithoutstockinfo,$LDDrugName,$LDAmountofDrugsused,$LDCostofdrugsused,$LDUnitPrice,$LDPartCode;
	...
	function DisplayMonthlyLabRequestsSummary($start_timeframe, $end_timeframe, $admission){
		...
		if ($res_array = $rs_ptr->GetArray()) {
			...
			if (!$PRINTOUT) {
				...
				if ($current_day > time()) {
					if (!$PRINTOUT) $td_bg_color="#ffffff";
					$italic_tag_open="<i>";
					$italic_tag_close="</i>";
				} else {
					if (!$PRINTOUT)$td_bg_color=$bg_color;
					$italic_tag_open="";
					$italic_tag_close="";
				} // end of if ($current_day > time())
...
function SetReportingLink_Admissions($tbl1, $tbl1_key, $tbl1_key1, $tbl2, $tbl2_key, $start, $end, $admission) {
	...
	$this->setTable($this->tmp_tbl_name.=time());
      $this->sql="CREATE  TEMPORARY TABLE $this->coretable ENGINE=MEMORY(SELECT $tbl1.encounter_nr,$tbl1.encounter_date,$tbl1.encounter_class_nr,$tbl1.current_ward_nr,
 ...
 //***********************************************************************************************************************************************
  function Detailed_Revenue($date_from,$date_to,$company,$admission){
          //echo $date_from.'=>'.$date_to.'=>'.$company.'=>'.$admission.'<br>';
          global $LDBilled_date,$LDAdmission_date,$LDPatient,$LDBirthDate,$LDSelianfilenumber,$LDMembership_NR,$LDForm_NR,$LDDescription,$LDGroup,$LDPrice,$LDRefund,$LDTopup,$LDDeposit;
          ...
          switch ($admission){
           ...
           //inpatient
               case 1:
                  $admission= 'encounter_class_nr=1';
                  break;
              //outpatient
              case 2:
                  $admission='encounter_class_nr=2';
                   break;

               //all
              case 0:
                 $admission='encounter_class_nr>0';
               break;
          ...
          $sql_insurance="CREATE TEMPORARY TABLE insurance_temp SELECT cp.pid,billelem.date_change AS billed_date,cp.name_first,cp.name_last,cp.date_birth,cp.selian_pid,cp.membership_nr,ce.form_nr,ce.encounter_date AS admission_date,billelem.description,ctzpricelist.item_number,ctzpricelist.partcode,billelem.price,billelem.amount,ctzpricelist.purchasing_class,(billelem.amount * billelem.price) AS total_price, billelem.insurance_id,ce.encounter_class_nr FROM care_person AS cp INNER JOIN care_encounter AS ce ON ce.pid=cp.pid INNER JOIN care_tz_billing_archive AS billarchive ON billarchive.encounter_nr=ce.encounter_nr INNER JOIN care_tz_billing_archive_elem AS billelem ON billarchive.nr=billelem.nr INNER JOIN care_tz_drugsandservices AS ctzpricelist ON ctzpricelist.item_id=billelem.item_number WHERE billelem.date_change BETWEEN $date_from_timestamp AND $date_to_timestamp $company";
          $sql_insurance_result=$db->Execute($sql_insurance);    
          
          
                  
          
          $sql_deposit="CREATE TEMPORARY TABLE deposit_temp SELECT  cp.pid, cp.name_first, cp.name_last,cp.date_birth,cp.membership_nr,ce.form_nr,cp.selian_pid,ce.encounter_nr,ce.encounter_class_nr,cbae.insurance_id,ce.encounter_date,cbae.date_change AS billed_date, cbae.description, cbae.amount,cbae.price FROM care_person AS cp INNER JOIN care_encounter AS ce ON cp.pid = ce.pid INNER JOIN care_tz_billing_archive AS cba ON ce.encounter_nr = cba.encounter_nr INNER JOIN care_tz_billing_archive_elem AS cbae ON cbae.nr = cba.nr WHERE (cbae.description='Advance' OR cbae.description='Topup' OR cbae.description='Refund') AND cbae.date_change BETWEEN '".$date_from_timestamp."' AND '".$date_to_timestamp."'";
          $sql_deposit_result=$db->Execute($sql_deposit); 
          
          $company_array=explode("AND billelem.insurance_id=",$company);
          $company_exploded=$company_array[1];       
          
          
          $sql_adv="SELECT  * FROM deposit_temp WHERE insurance_id=$company_exploded AND $admission  AND description='Advance'";
          $sql_topup="SELECT  * FROM deposit_temp WHERE insurance_id=$company_exploded AND $admission AND description='Topup'";
          $sql_refund="SELECT * FROM deposit_temp WHERE insurance_id=$company_exploded AND $admission AND description='Refund'";
          ...
          $sql_insurance_temp="SELECT pid,billed_date,admission_date,name_first,name_last,date_birth,selian_pid,amount,partcode,membership_nr,form_nr,description,price,purchasing_class,total_price FROM insurance_temp WHERE $admission ORDER BY admission_date ASC ";
          $sql_insurance_temp_result=$db->Execute($sql_insurance_temp);
         
          
          $sql_insurance_temp_group="SELECT purchasing_class,sum(total_price) AS total FROM insurance_temp WHERE $admission GROUP BY purchasing_class ";
          $insurance_temp_group_result=$db->Execute($sql_insurance_temp_group);
          
         $company_name="SELECT name FROM care_tz_company WHERE id = $company_exploded";
...
$filename=fopen('./gui/downloads/detailed_revenue.csv','w');  
 $date_from=date('d-m-Y H:i:s',$date_from_timestamp); 
 $date_to=date('d-m-Y H:i:s',$date_to_timestamp);
 fputcsv($filename,array('FROM:',$date_from,'TO:',$date_to,$company_names)); 
...
fputcsv($filename, array($LDBilled_date,$LDAdmission_date,$LDPatient,$LDBirthDate,$LDSelianfilenumber,$LDMembership_NR,$LDForm_NR,$LDPartCode,$LDDescription,$LDGroup,$LDNoOfItems,$LDPrice));
...
fputcsv($filename,array($date,$admission_date,$first_name.' '.$last_name,$birth_date,$registration_nr,$membership_nr,$form_nr,$partcode,$item_name,$group,$qty,$total));
}
...
echo '<tr bgcolor="lightgrey"><td colspan="12" align="center">PATIENT DEPOSIT</td></tr>';
$depost= 'PATIENT DEPOSIT';
fputcsv($filename,array($depost));
...
while($rows_adv=$result_adv->FetchRow()){
    ...
    <td>&nbsp;'.$LDDeposit.'</td>     
    <td>&nbsp;'.$LDDeposit.'</td>
    <td>&nbsp;'.$rows_adv['amount'].'</td>
    <td>&nbsp;'.number_format($rows_adv['price']).'</td>
       
  </tr>';
    $total_deposit+=$rows_adv['price'];
    
    $adv_bill_date         =date('j-m-Y',$rows_adv['billed_date']);
    $adv_encounter_date    =date('j-m-Y',strtotime($rows_adv['encounter_date'] ));
    $adv_names             =$rows_adv['name_first'].' '.$rows_adv['name_last'];
    $adv_date_birth        =date('j-m-Y',strtotime($rows_adv['date_birth']));
    $adv_file_nr           =$rows_adv['selian_pid'];
    $adv_membership_number =$rows_adv['membership_nr'];
    $adv_form_number       =$rows_adv['form_nr'];
    $adv_qty               =$rows_adv['amount']; 
    $adv_price             =$rows_adv['price'];       
    
     
fputcsv($filename,array($adv_bill_date,$adv_encounter_date,$adv_names,$adv_date_birth,$adv_file_nr,$adv_membership_number,$adv_form_number,$LDNA,$LDDeposit,$LDDeposit,$adv_qty,$adv_price));	 
	}
...
while($rows_topup=$result_topup->FetchRow()){
    ...
    <td>&nbsp;'.$LDTopup.'</td>
    <td>&nbsp;'.$LDTopup.'</td>  
    <td>&nbsp;'.$rows_topup['amount'].'</td>
    <td>&nbsp;'.number_format($rows_topup['price']).'</td>
       
  </tr>';	
	$total_deposit+=$rows_topup['price'];
	
	$tpp_bill_date           =date('j-m-Y',$rows_topup['billed_date']);
	$tpp_encounter_nr        =date('j-m-Y',strtotime($rows_topup['encounter_date'] ));
	$tpp_names               =$rows_topup['name_first'].' '.$rows_topup['name_last'];
	$tpp_date_birth          =date('j-m-Y',strtotime($rows_topup['date_birth']));
	$tpp_file_nr             =$rows_topup['selian_pid'];
	$tpp_member_nr           =$rows_topup['membership_nr'];
	$tpp_form_nr             =$rows_topup['form_nr'];
	$tpp_qty                 =$rows_topup['amount'];
	$tpp_price               =$rows_topup['price']; 
fputcsv($filename,array($tpp_bill_date,$tpp_encounter_nr,$tpp_names,$tpp_date_birth,$tpp_file_nr,$tpp_member_nr,$tpp_form_nr,$LDNA,$LDTopup,$LDTopup,$tpp_qty,$tpp_price));	     
}
...
while($rows_refund=$result_refund->FetchRow()){
    ...
    <td>&nbsp;'.$LDRefund.'</td>
    <td>&nbsp;'.$LDRefund.'</td>  
    <td>&nbsp;'.$rows_refund['amount'].'</td>
    <td>&nbsp;'.number_format($rows_refund['price']).'</td>
       
  </tr>';	
	$total_deposit+=$rows_refund['price'];
	
	$rf_bill_date       =date('j-m-Y',$rows_refund['billed_date']);
	$rf_encounter_date  =date('j-m-Y',strtotime($rows_refund['encounter_date'] ));
	$rf_names           =$rows_refund['name_first'].' '.$rows_refund['name_last'];
	$rf_date_birth      =date('j-m-Y',strtotime($rows_refund['date_birth']));
	$rf_file_nr         =$rows_refund['selian_pid'];
	$rf_member_nr       =$rows_refund['membership_nr'];
	$rf_form_nr         =$rows_refund['form_nr'];
	$rf_qty             =$rows_refund['amount'];
	$rf_price           =$rows_refund['price']; 
		
	fputcsv($filename,array($rf_bill_date,$rf_encounter_date,$rf_names,$rf_date_birth,$rf_file_nr,$rf_member_nr,$rf_form_nr,$LDNA,$LDRefund,$LDRefund,$rf_qty,$rf_price));
	
}

echo '<tr bgcolor="lightgrey"><td colspan="11" align="center"><h2>GRAND TOTAL:</h2></td><td><h2>'.number_format($total_all+$total_deposit).'</h2></td></tr>';
 fputcsv($filename,array('GRAND TOTAL:','','','','','','','','','',$total_all+$total_deposit));
echo '</table>';
?>  
<?php
...
echo $company_names;
 ...
 ?>         
          <p>FROM:<?php echo date('d-m-Y H:i:s',$date_from_timestamp).'   ';  ?> TO:<?php echo date('d-m-Y H:i:s',$date_to_timestamp);?></p>
...
<?php
 fputcsv($filename,array('','SUMMARY:','FROM:',$date_from,'TO:',$date_to,$company_names,''));
 fputcsv($filename,array($LDLab,$LDDrugs,$LDRadilogy,$LDDental,$LDEye,$LDMinProc,$LDProcSurg,$LDServicesTotal,$LDConsum,$LDSuppliesLab,$LDDeposit,$LDtotal));
 fputcsv($filename,array($lab_total,$all_drugs_total,$radio_total,$dental_total,$macho_total,$minor_proc_op_total,$surgical_op_total,$services_total,$supplies_total,$supplies_laboratory_total,$total_deposit,$grand_total));
