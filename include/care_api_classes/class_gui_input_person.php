					...
					<select name="region" size="1" onChange="getDistricts(this.value,\'dstr\');//redirect(this.options.selectedIndex)">
						...
						print ' <option value="27" id="27" style="background:yellow; font-weight:bold;">--Unknown--</option>';
							...
							while($catchment_area_row=$catchment_area_obj->FetchRow())
							{
									...
									if (isset($_POST['region']) and $_POST['region']==$catchment_area_row['region_id'])
												echo '<option value="'.$catchment_area_row['region_id'].'" selected="True" >'.$catchment_area_row['region_name'].'</option>';
									else echo  '<option value="'.$catchment_area_row['region_id'].'">'.$catchment_area_row['region_name'].'</option>';
			...
			<tr>
				...
				<td  class="reg_input" colspan=1 id="dstr">
							...
							<?php
								...
								$sql="SELECT * FROM care_tz_district WHERE district_id=".$_POST['district'];
								$result=$db->Execute($sql);
								if($row=$result->FetchRow()){
									$distr_name=$row['district_name'];
									}
							?>
							
								<option value="<?php echo $_POST['district'];?>" ><?php echo $distr_name;?></option>
						    <?php
							}
							?>
							</select>
				...
				<td  class="reg_input" colspan=1 id="wrd">
					<select name="ward" id="ward" size="1">
							...
							<?php
							 ...
							 $sql_ward="SELECT * FROM care_tz_ward WHERE ward_id=".$_POST['ward'];
							 $ward_result=$db->Execute($sql_ward);
							 if($row_ward=$ward_result->FetchRow()){
								 $ward_name=$row_ward['ward_name'];
								 }	
							?>
							<option value="<?php echo $_POST['ward'];?>" ><?php echo $ward_name;?></option>
						    <?php
							}
							?>
